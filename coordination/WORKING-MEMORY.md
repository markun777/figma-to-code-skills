...
- **Quality gates before PR close**: run `npm run gate` in `agentic-browser-ui` before marking a component case closed. Two gates: `gate:empty-svgs` (catches empty SVG exports without slices-name-map exemption note) and `gate:external-urls` (catches external image URLs in verify cards that can be blocked by ORB). See `inventory/quality-gates.md` for details.
